...
import numpy as np
from shotnoise import Hawkes_Shot_Noise
from tools import DataBox


class Hawkes_Shot_Noise_Simulate(Hawkes_Shot_Noise):
    def __init__(
        self, dim_endo: int, dim_exo: int, end_time: float, verbose: bool = True
    ):
    ...
    def simulate_immigrants(self):
        ...
        self.immigrant_labels = immigrant_labels
    ...
    def simulate_one(self, seed=37):
        ...
        Returns:
            ...
            origines[i] = list of origines of events of type i, 0 if endogenous, 1 if exogenous
        """
        print(seed)
        ...
        if self.immigrants is None:
            self.simulate_immigrants()
        ...
        for j in range(self.dim_endo):
            immigrants_j = self.immigrants[j]
            timestamps[j] += immigrants_j
            origines[j] += self.immigrant_labels[j]
            # -------------------------------------------------------
                ...
                for i in range(self.dim_endo):
                    ...
                    if self.verbose:
                        ...
                        if n_events > self.exp_descendants.sum() * next_print / 10:
                            print("%i%% finished" % (next_print * 10))
                            next_print += 1
        if self.verbose:
            print("100% finished")
